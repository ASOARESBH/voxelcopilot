...
--
-- Execute APÓS o schema principal e APÓS 2026-07-08_fix_tenant_nullable.sql
-- ============================================================
SET NAMES utf8mb4;
SET FOREIGN_KEY_CHECKS = 0;

-- ── 1. Adiciona colunas de seções estruturadas em cop_templates ──────────────
-- Permite armazenar seções separadas (técnica, análise, impressão) além do corpo
ALTER TABLE `cop_templates`
    ADD COLUMN IF NOT EXISTS `secao_tecnica`    LONGTEXT NULL AFTER `corpo`,
    ADD COLUMN IF NOT EXISTS `secao_analise`    LONGTEXT NULL AFTER `secao_tecnica`,
    ADD COLUMN IF NOT EXISTS `secao_impressao`  LONGTEXT NULL AFTER `secao_analise`,
    ADD COLUMN IF NOT EXISTS `secao_adicional`  LONGTEXT NULL AFTER `secao_impressao`,
    ADD COLUMN IF NOT EXISTS `origem`           VARCHAR(30) NOT NULL DEFAULT 'manual' AFTER `secao_adicional`,
    ADD COLUMN IF NOT EXISTS `origem_arquivo`   VARCHAR(255) NULL AFTER `origem`,
    ADD COLUMN IF NOT EXISTS `tags`             VARCHAR(500) NULL AFTER `origem_arquivo`,
    ADD COLUMN IF NOT EXISTS `publico`          TINYINT(1) NOT NULL DEFAULT 0 AFTER `tags`;
...
-- Armazena as máscaras do DOCX como biblioteca do sistema (disponível para todos)
CREATE TABLE IF NOT EXISTS `cop_mascaras_biblioteca` (
    ...
    FULLTEXT INDEX `ft_busca` (`titulo`, `nome_amigavel`, `corpo`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
