 ...
 *   storage/logs/pacs-YYYY-MM-DD.log      — eventos do PACS (assumir, laudo)
...
class Logger
{
    ...
    /**
     * Log específico de eventos do PACS (assumir exame, laudo recebido).
     * Grava em storage/logs/pacs-YYYY-MM-DD.log
     */
    public static function pacs(string $level, string $message, array $context = []): void
    {
        self::write(strtoupper($level), 'pacs', $message, $context);
        if (strtoupper($level) === 'ERROR') {
            self::write('ERROR', 'error', '[PACS] ' . $message, $context);
        }
    }

    /**
     * Log específico de webhooks recebidos/enviados.
    ...
    /**
     * Log específico da integração Copilot (autorização, token, sync).
