...
class Auth {
    ...
    /**
     * Autenticar usuário (superadmin ou médico)
     */
    public static function login(string $email, string $password): bool {
        $pdo  = Database::getInstance();
        $stmt = $pdo->prepare("SELECT * FROM cop_users WHERE email = :email AND status = 'ativo' LIMIT 1");
        ...
        $pdo->prepare("UPDATE cop_users SET ultimo_login = NOW() WHERE id = :id")
            ->execute(['id' => $user->id]);
        ...
        // Médico: busca tenants vinculados
        $stmt2 = $pdo->prepare("
            SELECT ut.tenant_id, ut.role, t.nome, t.status
            FROM cop_user_tenants ut
    ...
    public static function isImpersonating(): bool {
        return isset($_SESSION['impersonating_tenant_id']);
    }

    public static function impersonatingTenantId(): ?int {
        return isset($_SESSION['impersonating_tenant_id'])
            ? (int) $_SESSION['impersonating_tenant_id']
            : null;
    }
}
